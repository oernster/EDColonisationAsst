...
>
> EDCA makes no outbound request for colonisation data. Beyond the loopback probes it runs against itself, the only call it ever makes is the update check: one anonymous GET to the public GitHub releases API, asking what the latest published version is, carrying no identifier and no journal data. When that version is newer the user is offered the asset matching the platform they are running (falling back to the releases page), the chance to skip that version or to decide later.
>
> **There is exactly one update check in the product and it belongs to the tray.** [`update_check.py`](backend/src/runtime/update_check.py:1) fires it once per run, shortly after the tray appears, never repeating; a skipped version is remembered in [`update_state.py`](backend/src/services/update_state.py:1)'s per-user file so it is not raised again. The tray additionally offers a manual check from Help then Check for Updates, which ignores the skip and reports every outcome.
>
