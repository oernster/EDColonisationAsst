...
>
> **There is exactly one automatic check and it belongs to the tray.** [`update_check.py`](backend/src/runtime/update_check.py:1) fires it once per run, shortly after the tray appears, never repeating; a skipped version is remembered in [`update_state.py`](backend/src/services/update_state.py:1)'s per-user file so it is not raised again. Both surfaces additionally offer a manual check that ignores the skip and reports every outcome: Help then Check for Updates in the tray; the About tab in the web UI ([`useUpdateCheck.ts`](frontend/src/hooks/useUpdateCheck.ts:1), which has no timer of its own).
>
> Two things decided that split. The web UI is designed to be read from a tablet on the local network, so an offer made there would hand a Windows installer to a device that cannot run it, while the tray is on the machine EDCA is installed on. And two surfaces checking on their own timing meant one release could raise two prompts, each with a skip the other could not see. The browser therefore keeps no skipped-version store at all: skipping exists to silence a check that speaks unbidden; the browser no longer has one.
