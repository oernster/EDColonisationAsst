"""Putting the payload on disk, then taking it off again, file by file.
...
from installer.constants import IGNORED_DIR_NAMES
...
def safe_destination(root: Path, destination: Path) -> Path:
    ...
    tree; a link in the staged payload or an install directory that is
    itself a link can, since both resolve to somewhere outside.
    """
...
def copy_tree(
    ...
    *,
    ...
    """Copy ``source`` into ``target``, overwriting; return the file count.
    ...
    for dirpath, dirnames, filenames in os.walk(source):
        ...
        for name in filenames:
            ...
            if link_check(entry):
                continue
            destination = safe_destination(target, target_root / name)
...
def _remove_files(
    ...
    """Delete one directory's files, reporting each; return the new count."""
    for name in names:
        try:
            (base / name).unlink()
        except OSError:
