...
Shortcuts are written through the Windows scripting host rather than through COM
bindings, so the compiled setup program pulls in nothing beyond PySide6 and the
standard library. Every step is best effort: a shortcut that cannot be written
must not fail an otherwise complete install. British spelling is used in
...
from installer.ops.payload import installed_icon

SHORTCUT_TIMEOUT_S = 15.0

# The trailing index selects the first icon in a multi-size .ico, which is what
# lets Windows pick the size it needs for the taskbar, search and the desktop.
ICON_INDEX = 0
...
def _escaped(value: object) -> str:
    """Return a value quoted safely for a PowerShell literal string."""
...
def shortcut_script(exe_path: Path, link: Path, icon: Path | None) -> str:
    ...
    if icon is not None:
        script += _ICON_CLAUSE.format(icon=_escaped(icon), index=ICON_INDEX)
...
def create_shortcut(
    ...
    *,
    ...
    try:
        ...
        return
    icon = installed_icon(exe_path.parent)
