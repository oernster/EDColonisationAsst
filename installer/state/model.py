...
which keeps this package free of any dependency on the payload. British
...
from installer.ops.paths import is_under_program_files
...
def usable_location(location: Path | None) -> Path | None:
    """Return a recorded location only when it is one this user can write to.

    An installation recorded under Program Files was made with elevation the
    current user may not have, so it is not adopted as the default target.
    """
    if location is None or is_under_program_files(location):
        return None
    return location
...
def detect(
    ...
    *,
    ...
    location = usable_location(installed_location(keys))
    present = (
