...
Two registrations live here, both per user so no step needs administrator
rights:
...
# Modify and Repair are both offered by the setup window, so neither is
# suppressed in the Apps list.
_FLAG_CLEAR = 0
...
def write_uninstall_entry(
    ...
    *,
    ...
    uninstall_command = f'"{uninstaller}" {UNINSTALL_FLAG}'
    with winreg.CreateKey(winreg.HKEY_CURRENT_USER, keys.uninstall_key) as handle:
        ...
        winreg.SetValueEx(handle, UNINSTALL_STRING, 0, winreg.REG_SZ, uninstall_command)
        # Modify re-opens the setup window, where Repair and the options live.
        winreg.SetValueEx(handle, MODIFY_PATH, 0, winreg.REG_SZ, f'"{uninstaller}"')
        ...
        winreg.SetValueEx(handle, NO_MODIFY, 0, winreg.REG_DWORD, _FLAG_CLEAR)
        winreg.SetValueEx(handle, NO_REPAIR, 0, winreg.REG_DWORD, _FLAG_CLEAR)
...
def autostart_command(exe_path: Path) -> str:
    """Return the command the Run entry holds.
...
def is_autostart_enabled(keys: RegistryKeys = DEFAULT_KEYS) -> bool:
    ...
    Nothing read this back before, so the setup window always showed the option
    unticked and a Repair then deleted the setting of a user who had turned it
    on.
    """
