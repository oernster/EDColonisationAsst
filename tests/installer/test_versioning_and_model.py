"""Version comparison and the installed-state model.

British spelling is used in comments. No em dashes appear anywhere.
"""

from __future__ import annotations

import sys
from pathlib import Path

import pytest

from installer.constants import ENV_PROGRAM_FILES, FALLBACK_VERSION
from installer.state.model import InstallState, classify, detect, usable_location
from installer.state.registry import (
    RegistryKeys,
    set_autostart,
    write_uninstall_entry,
)
from installer.state.versioning import (
    NEWER,
    OLDER,
    SAME,
    compare_versions,
    version_tuple,
)

_WINDOWS = "win32"
_LINUX = "linux"


def test_version_tuple_reads_the_numeric_parts() -> None:
    assert version_tuple("2.9.0") == (2, 9, 0)


def test_version_tuple_treats_a_part_with_no_digits_as_zero() -> None:
    assert version_tuple("2.9.0-rc") == (2, 9, 0)


def test_version_tuple_of_an_empty_string_is_zero() -> None:
    assert version_tuple("") == (0,)


def test_compare_versions_orders_older_same_and_newer() -> None:
    assert compare_versions("2.8.0", "2.9.0") == OLDER
    assert compare_versions("2.9.0", "2.9.0") == SAME
    assert compare_versions("2.10.0", "2.9.0") == NEWER


def test_classify_reports_not_installed_when_nothing_is_recorded() -> None:
    assert classify("2.9.0", None) == InstallState.NOT_INSTALLED


def test_classify_reports_upgrade_reinstall_and_downgrade() -> None:
    assert classify("2.9.0", "2.8.0") == InstallState.UPGRADE
    assert classify("2.9.0", "2.9.0") == InstallState.REINSTALL
    assert classify("2.8.0", "2.9.0") == InstallState.DOWNGRADE


def test_a_recorded_location_this_user_can_write_to_is_usable(
    tmp_path: Path,
) -> None:
    assert usable_location(tmp_path) == tmp_path


def test_no_recorded_location_is_not_usable() -> None:
    assert usable_location(None) is None


def test_a_program_files_location_is_not_adopted(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """It was written with elevation this user may not have."""
    monkeypatch.setattr(sys, "platform", _WINDOWS)
    monkeypatch.setenv(ENV_PROGRAM_FILES, str(tmp_path))

    assert usable_location(tmp_path / "EDCA") is None


def test_detect_reports_not_installed_with_no_registration(
    scratch_keys: RegistryKeys, tmp_path: Path
) -> None:
    snapshot = detect("2.9.0", tmp_path, keys=scratch_keys)

    assert snapshot.state == InstallState.NOT_INSTALLED
    assert snapshot.installed is False
    assert snapshot.install_dir == tmp_path
    assert snapshot.installed_version == ""
    assert snapshot.autostart is False


def test_detect_reads_the_recorded_installation(
    monkeypatch: pytest.MonkeyPatch, scratch_keys: RegistryKeys, tmp_path: Path
) -> None:
    monkeypatch.setattr(sys, "platform", _LINUX)
    install_dir = tmp_path / "installed"
    install_dir.mkdir()
    write_uninstall_entry(
        install_dir, install_dir / "setup.exe", "2.8.0", keys=scratch_keys
    )
    set_autostart(True, install_dir / "app.exe", scratch_keys)

    snapshot = detect("2.9.0", tmp_path, keys=scratch_keys)

    assert snapshot.state == InstallState.UPGRADE
    assert snapshot.installed is True
    assert snapshot.installed_version == "2.8.0"
    assert snapshot.install_dir == install_dir
    assert snapshot.autostart is True


def test_detect_treats_a_registration_whose_directory_has_gone_as_absent(
    scratch_keys: RegistryKeys, tmp_path: Path
) -> None:
    """A stale entry must not offer Repair against a directory that is not there."""
    missing = tmp_path / "gone"
    write_uninstall_entry(missing, missing / "setup.exe", "2.8.0", keys=scratch_keys)

    snapshot = detect("2.9.0", tmp_path, keys=scratch_keys)

    assert snapshot.state == InstallState.NOT_INSTALLED
    assert snapshot.install_dir == tmp_path


def test_detect_falls_back_when_the_bundle_carries_no_version(
    scratch_keys: RegistryKeys, tmp_path: Path
) -> None:
    snapshot = detect("", tmp_path, keys=scratch_keys)

    assert snapshot.bundled_version == FALLBACK_VERSION
